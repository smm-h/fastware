...
the floor. It is justified below by a test that reproduces the exact shape this
suite needs: fastware is a server framework, and its serve/serve_background/
supervise/granian-embed tests boot real listeners on kernel-assigned ports.
"""
...
def test_ephemeral_loopback_bind_round_trips():
    ...
    This suite runs real background servers. ``serve_background`` and the
    supervise/granian-embed tests bind ``("127.0.0.1", 0)`` and dial the
    result back over HTTP; ``fastware.server._find_free_port`` and
    ``_probe_health``'s threaded ``HTTPServer`` do the same. The kernel assigns
    the port at bind time, so the ``host:port`` pair an allowlist would have to
    name does not exist at configuration time -- ``stricttest_socket_allowlist``
    cannot express port 0. ``loopback = "allow"`` is the only stance that
    permits this shape, and off-machine egress stays denied.
    """
