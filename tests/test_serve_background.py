...
class TestCallableTargetBackground:
    ...
    # The zombie-reap regression this class used to pin with a wall-clock
    # assertion on a live granian stop now lives in test_server.py
    # (test_stop_returns_promptly_for_an_already_dead_child). It was moved
    # because granian children drop SIGTERM under system load a few percent of
    # the time (see the grace-window comment in server.py), which stalls stop()
    # for the whole grace window for reasons that have nothing to do with the
    # reap -- the assertion failed on load, not on the regression it named. The
    # replacement builds the zombie directly, so it pins the reap and nothing
    # else.
