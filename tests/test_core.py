...
class TestAsgiHelpers:
    def test_type_aliases_exist(self):
        """Scope, Receive, Send are importable from fastware."""
        from fastware import Scope, Receive, Send
...
class TestMiddlewareConstructorAPI:
    ...
    async def test_callable_middleware_factory(self, client_for):
        ...
        configured_mw = partial(header_middleware, header_name="X-Powered-By", header_value="fastware")
        app = create_app(router, middleware=[configured_mw])
        async with client_for(app) as client:
            resp = await client.get("/api/data")
        assert resp.status_code == 200
        assert resp.headers["x-powered-by"] == "fastware"
...
class TestWebSocketPathParams:
    ...
    async def test_ws_path_param_received(self):
        ...
        # fastware's send_json uses msgspec and sends via "bytes" key
        import msgspec
        assert msgspec.json.decode(sent[1]["bytes"]) == {"id": "abc123"}
...
class TestWebSocketHelper:
    ...
    async def test_echo_json(self):
        ...
        assert sent[0]["type"] == "websocket.accept"
        # fastware's send_json uses msgspec and sends via "bytes" key
        assert msgspec.json.decode(sent[1]["bytes"]) == {"msg": "hello"}
    ...
    def test_websocket_importable_from_fastware(self):
        """WebSocket is importable from fastware."""
        from fastware import WebSocket as WS
...
class TestFileResponse:
    ...
    def test_file_response_importable_from_fastware(self):
        """FileResponse is importable from fastware."""
        from fastware import FileResponse as FR
